...
int RunPythiaAnalysisCharm()
{
  ...
  unsigned long nEventRequested   = 999999;
  ...
  pythiaOptions.push_back( new TString("Next:numberCount = 100000") );            // print message every n events
  ...
  //pythiaOptions.push_back( new TString("Beams:eCM = 5000.") ); // 5 TeV pp
  //  pythiaOptions.push_back( new TString("Beams:eCM = 13000."); // 13 TeV pp
  pythiaOptions.push_back( new TString("HardQCD:gg2ccbar = on") );
  pythiaOptions.push_back( new TString("HardQCD:qqbar2ccbar = on") );
  // Prevent charm hadrons from decaying.
  pythiaOptions.push_back( new TString("ParticleDecays:limitTau0 = on") );
  pythiaOptions.push_back( new TString("ParticleDecays:tau0Max = 0" ) );
...
/*
 ...
 //pythiaOptions.push_back( new TString("SoftQCD:all = on") );
 // Allow total sigma = elastic/SD/DD/ND
 ...
 */
  ...
  pythiaConfig->standaloneMode      = true;   // use interactions generated by the geometry generator
  ...
  p2ConfigND->nBins_pt    = 20;
  p2ConfigND->min_pt      = 0.00;
  p2ConfigND->max_pt      = 100.0;
  p2ConfigND->nBins_eta   = 80;
  p2ConfigND->min_eta     = -4;
  p2ConfigND->max_eta     = 4;
...
//  limits.push_back(400.0); // 0-10%
  ...
  ParticleFilter* charmOnly     = new ParticleFilter(0.001, 100.0, -4.0, 4.0, 10.0, -10.0,ParticleFilter::Charm,ParticleFilter::AllCharges);


  particleFiltersPythiaGen.push_back( charmOnly   );
//  particleFiltersGlobalAna.push_back( aliceV0ParticleFilter  );
//  particleFiltersGlobalAna.push_back( aliceTpcParticleFilter );
  particleFiltersGlobalAna.push_back( charmOnly  );

  int analysisOptionP1 = 6;
  int analysisOptionP2 = 6;
  ...
  switch (analysisOptionP1)
    {
      ...
      case 6:  particleFiltersP1Ana.push_back( charmOnly      ); break;
    }
  ...
  switch (analysisOptionP2)
    {
      ...
      case 6:  particleFiltersP2Ana.push_back( charmOnly      ); break;
    }
