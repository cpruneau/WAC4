...
HadronGasHistograms::HadronGasHistograms(const TString & collectionName,
                                         HadronGasConfiguration * hadronGasConfiguration,
                                         HadronGas * _hadronGas,
                                         LogLevel  debugLevel)
:
Histograms(collectionName,hadronGasConfiguration,debugLevel),
hadronGas(_hadronGas)
{
  // no ops
}
...
void HadronGasHistograms::createHistograms()
{
  ...
  h_rho1ThVsMass    = createProfile(bn+TString("rho1ThVsMass"), ac.nMass,0.0, ac.maxMass,"Mass (GeV)","#rho_{1}", true,true,true);
  h_rho1AllVsMass   = createProfile(bn+TString("rho1AllVsMass"),ac.nMass,0.0, ac.maxMass,"Mass (GeV)","#rho_{1}", true,true,true);
  h_rho1VsMass      = createProfile(bn+TString("rho1VsMass"),   ac.nMass,0.0, ac.maxMass,"Mass (GeV)","#rho_{1}", true,true,true);

  h_rho1All         = createHistogram(bn+TString("rho1All"),nSpecies,        0.0, double(nSpecies+1),       "Species","#rho_{1}", true,true,true,false);
  h_rho1Th          = createHistogram(bn+TString("rho1Th"), nStableSpecies,  0.0, double(nStableSpecies+1), "Species","#rho1_{1}^{Th}", true,true,true,false);
  h_rho1            = createHistogram(bn+TString("rho1"),   nStableSpecies,  0.0, double(nStableSpecies+1), "Species","#rho_{1}", true,true,true,false);
  h_rho1ToRho1Th    = createHistogram(bn+TString("rho1ToRho1Th"),nStableSpecies,  0.0, double(nStableSpecies+1), "Species","#rho_{1}", true,true,true,false);
  h_rho1rho1        = createHistogram(bn+TString("rho1rho1"),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      "Species","Species","Pairs", true,true,true,false);
  h_rho1thrho1th    = createHistogram(bn+TString("rho1thrho1th"),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      "Species","Species","#rho_{1}^{Th}#rho_{1}^{Th}", true,true,true,false);
  h_rho2Corr        = createHistogram(bn+TString("rho2Corr"),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      "Species","Species","Correlated Pairs", true,true,true,false);
  h_rho2Uncorr      = createHistogram(bn+TString("rho2Uncorr"),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      "Species","Species","Uncorrelated Pairs", true,true,true,false);

  h_rho2            = createHistogram(bn+TString("rho2"),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      "Species","Species","#rho_{2}", true,true,true,false);

  h_C2              = createHistogram(bn+TString("C2"),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      "Species","Species","C_{2}", true,true,true,false);
  h_R2              = createHistogram(bn+TString("R2"),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      nStableSpecies,  0.0, double(nStableSpecies),
                                      "Species","Species","R_{2}", true,true,true,false);
  h_BF              = createHistogram(bn+TString("BF"),18,  0.0, 18.0, "Pairs","BF", true,true,true,false);
  ...
  for (int iSpecies=0; iSpecies<nSpecies; iSpecies++)
    ...
    h_rho1ThVsP[iSpecies] = createHistogram(histoName,ac.nP,ac.minP,ac.maxP,"p (GeV/c)","#rho_{1}^{Th} (c/GeV)", true,true,true,false);
    }
...
void HadronGasHistograms::fillRho1VsP(HadronGas & hadronGas, double volume)
{
  ...
  for (unsigned int iSpecies=0; iSpecies<particleTypes.size(); iSpecies++)
    ...
    double deltaP = (ac.maxP-ac.minP)/double(ac.nP);
    double p = 0.0;
    for (int iP=1; iP<=ac.nP; iP++)
      {
...
void HadronGasHistograms::fill(HadronGas & hadronGas)
{
  if (reportStart("HadronGasHistograms",getName(),"fill(HadronGas & hadronGas)"))
    ;
  double rho1th1, rho1th2, rho1_1, rho1_2, rho_2_12c, rho_2_12u, mass, g;
  double zero = 0.0;
  int iSpecies1;
  int iSpecies2;
  int iStableSpecies1;
  ...
  for (unsigned int iStableSpecies1=0; iStableSpecies1<nStableSpecies; iStableSpecies1++)
    ...
    iSpecies1 = particleTypes.findIndexForPdgCode(pdgCode1);
