...
HistogramCollection & HistogramCollection::operator=(const HistogramCollection & source)
{
  if (this!=&source)
    ...
    for (unsigned int iObject=0; iObject<source.size(); iObject++)
      {
      TH1* h0 = source.objects[iObject];
      if (!ptrExist("HistogramCollection::ctor()",h0)) return *this;
      TH1* h1 = (TH1*) h0->Clone();
...
{
  //if (reportDebug("append(...)")) cout << "Inserting histo " << h->GetName() << endl;
                                           ...
                                           bool  print)
{
  ...
  if (reportDebug(fct))
    cout << endl << "Creating  1D histo " << name << " nBins:" << n << " min_x:" << min_x << " max_x:" << max_x << endl;
  TH1 * h;
...
{
  TString fct = "createHistogram(...)";
  if (reportDebug(fct)) cout << endl << "Creating  1D histo " << name << " with " << n << " non uniform nBins:" << endl;
  TH1 * h;
                                           ...
                                           bool  print)
{
  TString fct = "createHistogram(...)";
  if (reportDebug(fct))
    cout << endl << "Creating  2D histo " << name << " n_x:" << n_x << " min_x:" << min_x << " max_x:" << max_x << " n_y:" << n_y << " min_y:" << min_y << " max_y:" << max_y<< endl;
  TH2 * h;
...

{
  TString fct = "createHistogram(...)";
  if (reportDebug(fct))
    cout << endl << "Creating  2D histo " << name << " with " << n_x << " vs " << n_y << " non uniform nBins:" << endl;
  TH2 * h;
                                              ...
                                              bool  print)
{
  ...
  if (reportDebug(fct))
    cout << endl << "Creating  1D profile " << name << " n_x:" << n_x << " min_x:" << min_x << " max_x:" << max_x << endl;
                                              ...
                                              bool  print)
{
  TString fct = "createProfile(...)";
  if (reportDebug(fct))
    cout << endl << "Creating  1D profile " << name << " w/ variabe size bins" << endl;
                                                ...
                                                bool  print)
{
  TString fct = "createProfile(...)";
  if (reportDebug(fct))
    cout << endl << "Creating  2D profile " << name
...
{
  TString fct = "addHistogramsToExtList(TList *list, bool all)";
  if (reportDebug(fct))
    cout << endl << "Adding histo to external list"  << endl;
...
void HistogramCollection::saveHistograms(TFile * outputFile, bool saveAll)
{
  ...
  for (unsigned int iObject=0; iObject<size(); iObject++)
    {
    if (reportDebug(fct)) cout << " Saving iObject: " << iObject  << endl;
...
{
  TString fct = "scale(double factor, bool scaleAll)";
  // cout <<  "SHIT SHIT SHIT SHIT HistogramCollection::scale(double factor, bool scaleAll)     Scaling (selected) histograms by factor : " << factor  << endl;
  for (unsigned int iObject=0; iObject<size(); iObject++)
    {
    //cout << "SHIT SHIT SHIT SHIT iObject:" << iObject << endl;
...
TH1 * HistogramCollection::loadH1(TFile * inputFile,const TString & histoName)
{
  ...
  if (!h)
    {
    if (reportError(fct)) cout << endl << "Could not load histogram "<< histoName << endl;
    return nullptr;
    }
  append(h,0,1,1,1);
  return h;
...
TH1 * HistogramCollection::clone(const TH1 * h1, const TString & histoName)
{
  ...
  if (!h)
    {
    if (reportError(fct)) cout << endl << "Could not clone histogram "<< h1->GetName() << endl;
    return nullptr;
    }
  h->SetName(histoName);
  return h;
